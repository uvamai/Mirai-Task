...
import { loginUser, logout, PasswordPolicyError, refreshTokens, registerUser } from '../services/authService';
import { authenticateJwt, loadMembership } from '../middleware/auth';
import { User, TenantSubscription, SubscriptionPlan, TenantUsage, Tenant } from '../models';
...
authRouter.get('/auth/me', authenticateJwt, loadMembership, async (req, res) => {
...
authRouter.patch('/auth/me/preferences', authenticateJwt, loadMembership, async (req, res) => {
  const { error, value } = patchMePreferencesSchema.validate(req.body, { abortEarly: false });
  if (error) {
    res.status(400).json({ error: 'Validation failed', details: error.details });
    return;
  }
